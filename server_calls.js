...
var cust;
/*(async () => {
  await req(options('GET', 'customers/' + initialCustomerId + '/accounts/' ))
    .then((resp) => {
      cust = resp.result.bankAccounts[0].balance;
      console.log(cust);
    }, handleError)
})();*/

/*(async () => {
  await req(options('GET', 'customers/' +  + 'transactions/'))
    .then((resp) => {
      cust = resp.result;
      console.log(cust);
    }, handleError)
